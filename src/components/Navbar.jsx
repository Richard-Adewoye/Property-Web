...
export default function Navbar({ onOpenAuth }) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#FAF9F6]/95 backdrop-blur-md border-b border-[#1A1A1A]/10 transition-all duration-200">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center md:hidden">
            <button
              ...
              className="p-2 border-2 border-[#1A1A1A] text-[#1A1A1A] hover:bg-[#1A1A1A] hover:text-white transition-colors"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#FAF9F6] border-b-2 border-[#1A1A1A] px-4 pt-3 pb-6 space-y-3 animate-in slide-in-from-top duration-200">
          <a
            href="#"
            onClick={() => setMobileMenuOpen(false)}
            className="block px-3 py-2 font-bold text-xs uppercase tracking-widest text-[#D13111]"
          >
            Home
          </a>
          <a
            href="#properties"
            onClick={() => setMobileMenuOpen(false)}
            className="block px-3 py-2 font-bold text-xs uppercase tracking-widest text-[#1A1A1A] hover:text-[#D13111]"
          >
            Buy & Rent Properties
          </a>
          <a
          ...
          <div className="pt-3 border-t border-[#1A1A1A]/10 flex flex-col gap-2">
            <button
              ...
              className="w-full text-center py-3 border-2 border-[#1A1A1A] text-[#1A1A1A] font-bold text-xs uppercase tracking-widest"
            >
              Join with Us
            </button>
            <button
